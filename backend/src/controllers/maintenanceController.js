    ...
    try {
        ...
        const options = {
            ...
            populate: [
                ...
                { path: 'relatedTicket', select: 'title status' }
            ]
        };

        const result = await paginate(Maintenance, query, options);
...
export const getAssetMaintenances = async (req, res, next) => {
    try {
        ...
        const result = await paginate(Maintenance, { asset: req.params.assetId }, options);
