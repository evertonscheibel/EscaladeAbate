...
export const getHistory = async (req, res, next) => {
    try {
        ...
        const options = {
            ...
            sort: { dt_entrada: -1 },
            populate: [
                { path: 'veiculo_id', select: 'placa tipo_veiculo' },
                { path: 'pessoa_id', select: 'nome' },
                { path: 'empresa_id', select: 'nome_fantasia' },
                { path: 'tipo_acesso_id', select: 'nome cor icone' }
            ]
        };

        const result = await paginate(GatehouseAccess, query, options);
