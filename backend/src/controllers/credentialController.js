    ...
    try {
        ...
        const options = {
            ...
            populate: [
                ...
                { path: 'relatedAsset', select: 'assetId description' },
                { path: 'relatedNetworkDevice', select: 'name ipAddress' }
            ]
        };

        const result = await paginate(Credential, filter, options);
...
export const updateCredential = async (req, res) => {
    try {
        ...
        // Whitelist de campos permitidos — elimina mass assignment
        const allowedFields = [
            'title', 'description', 'category', 'username', 'host', 'port', 'url',
            'visibility', 'allowedUsers', 'status', 'tags',
            'relatedAsset', 'relatedNetworkDevice', 'relatedSupplier',
            'passwordPolicy'
        ];
        allowedFields.forEach(f => {
            if (rest[f] !== undefined) credential[f] = rest[f];
        });
...
export const copyPassword = async (req, res) => {
    try {
        ...
        // Registrar cópia (Auditoria Central)
        await logAudit({
            user: req.user,
            ...
            severity: 'high',
            details: `Usuário copiou a senha da credencial: ${credential.title}`,
            req
        });
...
// @desc    Obter estatísticas de credenciais (Cacheadas)
// @route   GET /api/credentials/stats
// @access  Private (Admin)
export const getCredentialStats = async (req, res) => {
    try {
        const stats = await getCachedData('credential_stats_admin', async () => {
            // Total por categoria
