...
import DeboningPiece from '../models/DeboningPiece.js';
...
import { generateDeboningPDF } from '../utils/deboningPdfGenerator.js';
...
async function recalculateAllLots(scheduleId) {
    ...
    if (step1Ops.length > 0) await DeboningLot.bulkWrite(step1Ops);
    ...
    for (let i = 0; i < lots.length; i++) {
        ...
        // Se for horário manual, não recalcular
        if (lot.manualTimes) {
            step2Ops.push({
                updateOne: {
                    filter: { _id: lot._id },
    ...
    if (step2Ops.length > 0) await DeboningLot.bulkWrite(step2Ops);
...
async function updateScheduleTotals(scheduleId) {
    ...
    const totals = lots.reduce((acc, lot) => ({
        ...
        cupim: acc.cupim + (lot.production?.cupim || 0),
    ...
    }), { boi: 0, vaca: 0, novilha: 0, bubalino: 0, touro: 0, totalCarcassas: 0, traseiro: 0, dianteiro: 0, ponta: 0, cupim: 0, recortes: 0, osso: 0, sebo: 0, totalKg: 0 });
...
export const getCalendar = async (req, res, next) => {
    try {
        ...
        const monthlySummary = {
            totalBoi: schedules.filter(s => s.status === 'CLOSED').reduce((s, x) => s + (x.totalBoi || 0), 0),
            totalVaca: schedules.filter(s => s.status === 'CLOSED').reduce((s, x) => s + (x.totalVaca || 0), 0),
            totalNovilha: schedules.filter(s => s.status === 'CLOSED').reduce((s, x) => s + (x.totalNovilha || 0), 0),
            totalBubalino: schedules.filter(s => s.status === 'CLOSED').reduce((s, x) => s + (x.totalBubalino || 0), 0),
            totalTouro: schedules.filter(s => s.status === 'CLOSED').reduce((s, x) => s + (x.totalTouro || 0), 0),
            totalCarcassas: schedules.filter(s => s.status === 'CLOSED').reduce((s, x) => s + (x.totalCarcassas || 0), 0),
            totalProduzidoKg: schedules.filter(s => s.status === 'CLOSED').reduce((s, x) => s + (x.totalProduzidoKg || 0), 0),
            closedDays: schedules.filter(s => s.status === 'CLOSED').length
        };
...
export const getScheduleByDate = async (req, res, next) => {
    try {
        ...
        const schedule = await DeboningSchedule.findOne({
            ...
            path: 'lots',
            populate: [
                { path: 'slaughterLot', select: 'lotNumber rancherName brokerNumber' },
                { path: 'broker', select: 'name cpfCnpj' }
            ]
        ...
        // Buscar statuses reais de execução (PcpOp)
        const ops = await PcpOp.find({ programacaoId: schedule._id });
        const opMap = ops.reduce((map, op) => {
            map[op._id.toString()] = op;
            return map;
        }, {});

        const scheduleData = schedule.toObject();
        scheduleData.lots = scheduleData.lots.map(lot => {
            const op = opMap[lot._id.toString()];
            if (op) {
                return {
                    ...lot,
                    lotStatus: op.status, // Sobrescrever status com o do PcpOp
                    tempoProdutivoMin: op.tempoProdutivoMin,
                    tempoParadoMin: op.tempoParadoMin,
                    pecasPorHora: op.pecasPorHora,
                    startTimeReal: op.inicioReal,
                    endTimeReal: op.fimReal
                };
            }
            return lot;
        });
...
export const updateSchedule = async (req, res, next) => {
    try {
        ...
        const scheduleAllowedFields = ['startTime', 'targetCarcassesPerHour', 'breakfastTime', 'breakfastDuration', 'lunchTime', 'lunchDuration', 'notes', 'responsibleName', 'chamberTemperature'];
        const scheduleUpdateData = {};
        scheduleAllowedFields.forEach(f => { if (req.body[f] !== undefined) scheduleUpdateData[f] = req.body[f]; });

        const updated = await DeboningSchedule.findByIdAndUpdate(req.params.id, scheduleUpdateData, { new: true, runValidators: true });
...
export const importFromSlaughter = async (req, res, next) => {
    try {
        ...
        for (const sLot of slaughterLots) {
            ...
            const breakdown = {
                boi: { traseiro: (sLot.boi || 0) * 2, dianteiro: (sLot.boi || 0) * 2, ponta: (sLot.boi || 0) * 2, cupim: 0 },
                vaca: { traseiro: (sLot.vaca || 0) * 2, dianteiro: (sLot.vaca || 0) * 2, ponta: (sLot.vaca || 0) * 2, cupim: 0 },
                novilha: { traseiro: (sLot.novilha || 0) * 2, dianteiro: (sLot.novilha || 0) * 2, ponta: (sLot.novilha || 0) * 2, cupim: 0 },
                bubalino: { traseiro: (sLot.bubalino || 0) * 2, dianteiro: (sLot.bubalino || 0) * 2, ponta: (sLot.bubalino || 0) * 2, cupim: 0 },
                touro: { traseiro: (sLot.touro || 0) * 2, dianteiro: (sLot.touro || 0) * 2, ponta: (sLot.touro || 0) * 2, cupim: 0 },
            };
...
export const createLot = async (req, res, next) => {
    try {
        ...
        let timing = {};
        if (!req.body.manualTimes) {
            timing = calculateLotTiming(req.body, lastLot?.endTime || schedule.startTime, schedule);
        }
...
export const updateLot = async (req, res, next) => {
    try {
        ...
        const lotAllowedFields = ['boi', 'vaca', 'novilha', 'bubalino', 'touro', 'origin', 'broker', 'sifNumber', 'breakdown', 'observations', 'destino', 'plannedMarket', 'pesoMedioCarcassa', 'startTime', 'endTime', 'manualTimes', 'durationMinutes'];
        const lotUpdateData = {};
        lotAllowedFields.forEach(f => { if (req.body[f] !== undefined) lotUpdateData[f] = req.body[f]; });

        await DeboningLot.findByIdAndUpdate(req.params.id, lotUpdateData, { new: true, runValidators: true });
...
export const startSchedule = async (req, res, next) => {
    try {
        ...
        const lots = await DeboningLot.find({ schedule: schedule._id }).sort('order');
        if (lots.length === 0) {
            return res.status(400).json({ success: false, message: 'Programação sem lotes não pode ser iniciada' });
        }

        // Criar PcpOps para cada lote
        const opPromises = lots.map((lot, index) => {
            return PcpOp.create({
                _id: lot._id, // Usar o mesmo ID do lote para facilitar o link no frontend
                programacaoId: schedule._id,
                sequencia: index + 1,
                itemCorte: lot.origin,
                qtdPlanejada: lot.totalCarcassas,
                pesoPlanejadoKg: lot.totalCarcassas * lot.pesoMedioCarcassa,
                destino: lot.destino,
                status: 'PENDENTE',
                origemEntradaKg: lot.totalCarcassas * lot.pesoMedioCarcassa
            });
        });
...
export const closeSchedule = async (req, res, next) => {
    try {
        ...
        // Hook PCP
        try {
            const normalizedDate = new Date(schedule.scheduleDate);
            normalizedDate.setHours(0, 0, 0, 0);

            let pcpDay = await PcpDayPlan.findOne({ date: normalizedDate });
            if (!pcpDay) {
                pcpDay = new PcpDayPlan({
                    date: normalizedDate,
                    createdBy: req.user.id
                });
            }

            pcpDay.links.deboningScheduleId = schedule._id;
            pcpDay.totalDeboningCarcasses = schedule.totalCarcassas;

            // Aggregar destinos
            const aggregatedDestinos = await DeboningLot.aggregate([
                { $match: { schedule: schedule._id } },
                { $group: { _id: '$plannedMarket', total: { $sum: '$totalProduzidoKg' } } }
            ]);

            const realized = { MI: 0, EXP: 0, IND: 0 };
            aggregatedDestinos.forEach(d => {
                if (realized[d._id] !== undefined) realized[d._id] = d.total;
                else if (d._id === 'MI_EXP') {
                    realized.MI += d.total / 2;
                    realized.EXP += d.total / 2;
                }
            });

            pcpDay.realizedByMarket = realized;
            await pcpDay.save();
        } catch (pcpError) {
            console.error('Erro no hook PCP desossa:', pcpError);
        }
...
export const reopenSchedule = async (req, res, next) => {
    try {
        ...
        // Limpar execução do PCP
        await PcpOp.deleteMany({ programacaoId: id });
        await PcpEvento.deleteMany({ programacaoId: id });
...
// @desc    Exportar PDF da programação
// @route   GET /api/deboning/schedules/:id/pdf
export const exportSchedulePdf = async (req, res, next) => {
    try {
        ...
        const url = await generateDeboningPDF(schedule);
...
export const getPieces = async (req, res, next) => {
    try {
        const pieces = await DeboningPiece.find().sort('name');
...
export const createPiece = async (req, res, next) => {
    try {
        const piece = await DeboningPiece.create(req.body);
...
export const updatePiece = async (req, res, next) => {
    try {
        const piece = await DeboningPiece.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
...
export const deletePiece = async (req, res, next) => {
    try {
        await DeboningPiece.findByIdAndDelete(req.params.id);
