...
export const assignTicket = async (req, res) => {
    try {
        ...
        // Se o ticket estava 'aberto' mas não tinha dono, mantemos aberto. 
        // Se tivesse algum outro status de entrada, mantemos, mas garantimos que não seja nulo.
        ...
        await logEvent(id, 'ASSIGNED', managerId, {
            ...
            assignedToName: (await mongoose.model('User').findById(assignedToId).select('name'))?.name || 'Desconhecido',
            level: supportLevel
        });
...
export const acceptTicket = async (req, res) => {
    try {
        ...
        if (ticket.assignedTo && Array.isArray(ticket.assignedTo) && ticket.assignedTo.length > 0 &&
            !ticket.assignedTo.map(id => id.toString()).includes(agentId.toString())) {
...
export const getNextTicket = async (req, res) => {
    try {
        ...
        const agent = await mongoose.model('User').findById(agentId).select('name');
        await logEvent(updatedTicket._id, 'ASSIGNED', agentId, { method: 'PULL', assignedToName: agent?.name });
...
export const getAgentMetrics = async (req, res) => {
    try {
        ...
        // Se passar agentId por query e for admin/tecnico, permite ver métricas de outro
        if (req.query.agentId && (req.user.role === 'admin' || req.user.role === 'tecnico')) {
            if (req.query.agentId !== 'unassigned') {
                agentId = req.query.agentId;
            }
        }

        const objAgentId = new mongoose.Types.ObjectId(agentId);
        ...
        const stats = await Ticket.aggregate([
            // assignedTo é um array, então usamos $in or equality if comparing to ObjectId
            { $match: { assignedTo: objAgentId } },
            {
                $group: {
                    _id: null,
                    totalTickets: { $sum: 1 },
