...
export const getPurchaseOrders = async (req, res) => {
    try {
        ...
        const options = {
            ...
            sort: { createdAt: -1 },
            populate: [
                { path: 'purchaseRequest', select: 'requestNumber title department' },
                { path: 'supplier', select: 'name tradeName contact' },
                { path: 'quote' },
                { path: 'receivedItems.receivedBy', select: 'name' },
                { path: 'receivedItems.asset' }
            ]
        };

        const result = await paginate(PurchaseOrder, filter, options);
