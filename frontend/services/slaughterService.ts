import api from './api';
import { SlaughterSchedule, CalendarDay, SlaughterLot, Rancher, MonthlySummary } from '../types/slaughter';

export const slaughterService = {
    getCalendar: async (month: string): Promise<{ data: CalendarDay[], monthlySummary: MonthlySummary }> => {
    ...
    reorder: async (id: string, lotIds: string[]): Promise<SlaughterSchedule> => {
        const { data } = await api.post(`/slaughter/schedules/${id}/reorder`, { lotIds });
        return data.data;
    }
};
