...
interface SortableLotRowProps {
    ...
    timing?: {
        ...
        isBreak?: boolean;
        breakType?: 'CAFÉ' | 'ALMOÇO';
    };
}

const SortableLotRow: React.FC<SortableLotRowProps> = ({ lot, index, isDraft, onEdit, onDelete, timing }) => {
    ...
    return (
        <tr ref={setNodeRef} style={style}>
            ...
            <td className={`time-col ${timing?.isBreak ? 'break-indicator' : ''}`}>
                {timing?.startTime || '--:--'}
            </td>
            <td className="duration-col">{timing?.durationMinutes || '--'}</td>
            <td className="time-col">
                {timing?.endTime || '--:--'}
                {timing?.isBreak && <span className="break-label">{timing.breakType}</span>}
            </td>
...
export const SlaughterSchedule: React.FC = () => {
    ...
    const [confirmAction, setConfirmAction] = useState<{ type: 'CLOSE' | 'DELETE' | 'REOPEN', index?: number } | null>(null);
    ...
    const calculateLotTimings = useCallback(() => {
        const rate = parseInt(localRate) || 100;
        let currentMins = timeToMinutes(localStartTime);

        const bStart = timeToMinutes(breakfastTime);
        const bDur = parseInt(breakfastDuration) || 0;
        const lStart = timeToMinutes(lunchTime);
        const lDur = parseInt(lunchDuration) || 0;

        let appliedB = false;
        let appliedL = false;

        return localLots.map((lot) => {
            const lotTotal = (lot.boi || 0) + (lot.vaca || 0) + (lot.novilha || 0) + (lot.bubalino || 0) + (lot.touro || 0);
            const durationMins = Math.ceil((lotTotal / rate) * 60);

            // Verificar se o início coincide com intervalo
            if (!appliedB && currentMins >= bStart && currentMins < bStart + bDur) {
                currentMins = bStart + bDur;
                appliedB = true;
            }
            if (!appliedL && currentMins >= lStart && currentMins < lStart + lDur) {
                currentMins = lStart + lDur;
                appliedL = true;
            }

            const startStr = minutesToTime(currentMins);
            const rawEndMins = currentMins + durationMins;
            let finalEndMins = rawEndMins;

            let crossedBreak = false;
            let breakType: 'CAFÉ' | 'ALMOÇO' | undefined;

            // Verificar se o intervalo acontece DURANTE o abate do lote
            if (!appliedB && currentMins < bStart && rawEndMins > bStart) {
                finalEndMins += bDur;
                appliedB = true;
                crossedBreak = true;
                breakType = 'CAFÉ';
            }
            if (!appliedL && currentMins < lStart && rawEndMins > lStart) {
                finalEndMins += lDur;
                appliedL = true;
                crossedBreak = true;
                breakType = 'ALMOÇO';
            ...
            const timing = {
                startTime: startStr,
                endTime: minutesToTime(finalEndMins),
                durationMinutes: durationMins,
                isBreak: crossedBreak,
                breakType
            };

            currentMins = finalEndMins;
            return timing;
        });
    }, [localLots, localStartTime, localRate, breakfastTime, breakfastDuration, lunchTime, lunchDuration]);

    const lotTimings = calculateLotTimings();
    ...
    const handleBulkSend = async (targetStatus: 'DRAFT' | 'ENVIADA' = 'ENVIADA') => {
        if (!date || localLots.length === 0) {
            alert('Não há lotes para salvar.');
            return;
        }

        if (targetStatus === 'ENVIADA' && !showConfirmModal) {
            setConfirmAction({ type: 'CLOSE' });
            setShowConfirmModal(true);
            return;
        }

        if (targetStatus === 'DRAFT' && schedule?.status === 'ENVIADA' && !showConfirmModal) {
        ...
        try {
            ...
            console.log('[DEBUG] Salvando pré-escala para data:', date, 'Status:', targetStatus, 'Payload:', payload);
            ...
            if (response.success && response.data) {
                ...
                alert(targetStatus === 'ENVIADA' ? 'Pré-escala FECHADA com sucesso!' : 'Rascunho salvo com sucesso!');
            } else {
        ...
        <div className="slaughter-schedule">
            ...
            {showConfirmModal && (
                <StandardFormModal
                    ...
                    title={
                        confirmAction?.type === 'CLOSE' ? 'Fechar Pré-Escala' :
                        confirmAction?.type === 'REOPEN' ? 'Reabrir Pré-Escala' : 'Confirmar Exclusão'
                    }
                    icon={<Clock size={20} />}
                    size="md"
                    footer={
                        <div className="form-actions">
                            ...
                            <button 
                                className="btn-primary" 
                                style={{ backgroundColor: confirmAction?.type === 'DELETE' ? '#dc3545' : '#28a745' }}
                                onClick={() => {
                                    if (confirmAction?.type === 'CLOSE') {
                                        setShowConfirmModal(false);
                                        setConfirmAction(null);
                                        handleBulkSend('ENVIADA');
                                    } else if (confirmAction?.type === 'REOPEN') {
                                        setShowConfirmModal(false);
                                        setConfirmAction(null);
                                        handleBulkSend('DRAFT');
                                    } else if (confirmAction?.type === 'DELETE' && confirmAction.index !== undefined) {
                                        const updatedLots = localLots.filter((_, i) => i !== confirmAction.index);
                                        setLocalLots(updatedLots);
                                        setShowConfirmModal(false);
                                        setConfirmAction(null);
                                    }
                    ...
                    <div style={{ padding: '20px', textAlign: 'center' }}>
                        <p style={{ fontSize: '16px', color: 'var(--text-secondary)' }}>
                            {confirmAction?.type === 'CLOSE' 
                                ? 'Deseja realmente FECHAR esta pré-escala? Após fechar, as edições serão bloqueadas e ela ficará disponível para o SIF.' 
                                : confirmAction?.type === 'REOPEN'
                                ? 'Deseja REABRIR esta pré-escala para edição?'
                                : 'Deseja realmente excluir este lote do rascunho?'}
                        </p>
                    </div>
                </StandardFormModal>
            )}
