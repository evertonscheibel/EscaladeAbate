        ...
        <div className="pcp-day-container">
            <header className="pcp-header">
                <div className="header-left">
                    ...
                    <div className="header-title">
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <ArrowRightLeft color="var(--primary)" size={24} />
                            <h1>Controle Diário - {format(new Date(date!), 'dd/MM/yyyy', { locale: ptBR })}</h1>
                        </div>
                        <span className={`deb-new-badge ${plan.status.toLowerCase() === 'draft' ? 'pendente' : plan.status.toLowerCase() === 'in_progress' ? 'em-processo' : 'concluido'}`} style={{ marginTop: '8px' }}>
                ...
                <div className="header-actions" style={{ display: 'flex', gap: '12px' }}>
                    {plan.status === 'DRAFT' && (
                        <button className="deb-new-btn-primary" onClick={handleStart}>
                            <Play size={18} fill="currentColor" /> Iniciar Dia
                        </button>
                    )}
                    <button className="deb-new-btn-outline" onClick={() => navigate('/pcp/reports')}>
