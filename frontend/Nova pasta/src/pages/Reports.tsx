...
export const Reports: React.FC = () => {
    ...
    const loadReports = async () => {
        try {
            ...
            const [assetRes, maintenanceStatsRes, maintenanceReportRes, operationalRes, alertsRes, agentRes, agentActivityRes] = await Promise.all([
                ...
                ticketService.getAgentActivityReport(startDate, endDate)
            ]);
            ...
            const activityData = Array.isArray(agentActivityRes) ? agentActivityRes : (agentActivityRes?.data || []);
            setAgentActivityReport(Array.isArray(activityData) ? activityData : []);
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px', alignItems: 'center' }}>
                        <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                            <h2 style={{ fontSize: '20px', fontWeight: '700', color: '#1e293b', margin: 0 }}>
                                {period === 'analytical' ? 'Relatório Analítico de Tickets' : 'Produtividade da Equipe'}
                            </h2>
                            <div style={{ display: 'flex', background: '#f1f5f9', borderRadius: '8px', padding: '4px' }}>
                                <button
                                    onClick={() => setPeriod(prev => prev === 'analytical' ? '30' : prev)}
                                    style={{
                                        padding: '6px 12px',
                                        borderRadius: '6px',
                                        border: 'none',
                                        background: period !== 'analytical' ? 'white' : 'transparent',
                                        boxShadow: period !== 'analytical' ? '0 1px 2px rgba(0,0,0,0.1)' : 'none',
                                        fontSize: '13px',
                                        fontWeight: '600',
                                        color: period !== 'analytical' ? '#1e293b' : '#64748b',
                                        cursor: 'pointer'
                                    }}
                                >
                                    Resumido
